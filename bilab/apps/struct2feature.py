...
class Point(object):
    ...
    def __init__(self, identifier=None, name=None, value=0.0):
        ...
        # self.__identifier = (str(uuid.uuid1())
        #         if identifier is None else sanitize_id(str(identifier)))
        self.name = name
        self.value = value

    def __str__(self):
        return "{}, name={}, value={}".format(self.__identifier,
...
def NDRmapping(X, pdbid, chain, save_mapping_file, no_dims=1,
               iters=100, perplexity=30.0, verbose=False):
    """
    ...
    if not isinstance(X, (np.ndarray, np.generic)):
        raise TypeError("X: Inappropriate argument type for {}".format(
    ...
    if X.ndim == no_dims:
        print("The dimension of the input and output is equal. "
    ...
    if verbose:
        ...
        print("NDRmapping (sampling,{}) : elapsed time:{}".format(
              iters, elapsed_time))
    results = []
    if save_mapping_file:
        ...
        for i, residue in enumerate(chain.iterResidues()):
            res = "{}_{}_{}".format(residue.getResname(),
                                    residue.getChid(), residue.getResnum())
            results.append(Point(name=res, value=Y[i]))
            resInOne = bilab.utilities.utilities.three2one(
                        residue.getResname())
            data.append([pdbid, residue.getChid(), residue.getResnum(),
                        residue.getResname(), resInOne, dY[i]])
        header_comment =\
            "Mapping ({},{}) - No_dims:{}, Sampling:{}, Perplexity:{}\n".format(
                    N, N, no_dims, iters, perplexity)
        np.savetxt(save_mapping_file, data,
                   fmt="%4s %1s %6s %3s %1s %15s", header=header_comment)
    else:
        for i, residue in enumerate(chain.iterResidues()):
            res = "{}_{}_{}".format(residue.getResname(),
                                    residue.getChid(), residue.getResnum())
...
def saveMat2png(*args, **kwargs):
    ...
    if ncols == 1:
        ...
        for ax_num in xrange(ncols):
            cax = axs[ax_num].matshow(args[ax_num],
                                      cmap=colmap, interpolation='none')
...
def main(argv):
    ...
    # Load pdbs
    mol, pdb_header = bilab.structure.parsePDB(pdb_f,
                                               title=pdbid, header=True)
    if pdb_header["identifier"] != pdbid:
        print(
            "Input pdbid({}) is inconsistent with the identifier ({}) in the pdb file.".format(pdbid, pdb_header["identifier"]))
