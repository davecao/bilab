...
call_correction_function_py(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &configuration))
    return NULL;
  ...
  */
  ...
  case 1:
    while (universe->state_access < 0) {
      ...
      for (i = 0; i < MMTK_MAX_THREADS; i++) {
        if (universe->state_access_type[i] == 0) break;
      }
#if THREAD_DEBUG
...
init_MMTK_universe(void)
{
  ...
  /* Create the module */
  m = Py_InitModule("_MMTK_universe", universe_module_methods);
