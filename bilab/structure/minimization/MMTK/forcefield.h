...
typedef int gradient_function(struct ffedata *energy, int i, vector3 gradient);

typedef int fc_function(struct ffedata *energy, int i, int j, tensor3 fc, 
                      double r_sq);
...
{ \
  PyObject *module = PyImport_ImportModule("bilab.structure.minimization._MMTK_forcefield"); \
  if (module != NULL) { \
