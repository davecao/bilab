...
energyterm_getattr(PyFFEnergyTermObject *self, char *name)
{
  if (strcmp(name, "name") == 0) {
    ...
    int i;
    for (i = 0; i < self->nterms; i++)
      PyTuple_SetItem(ret, (Py_ssize_t)i,PyString_FromString(self->term_names[i]));
    return ret;
  }
...
static void
evaluator(PyFFEvaluatorObject *self, energy_data *energy,
          PyArrayObject *coordinates, int small_change)
{
  ...
  if (self->communicator != NULL && input.nprocs > 1) {
    ...
    if (PyMPI_Share(self->communicator, energy->energy_terms,self->energy_parts,
...
nblist_update_py(PyObject *self, PyObject *args)
{
  ...
  nblist_update(nblist,  array->dimensions[0], (double *)array->data, geometry_data);
  Py_INCREF(Py_None);
  return Py_None;
}
...
#endif
PyMODINIT_FUNC
init_MMTK_forcefield(void)
{
  PyObject *m, *d, *module, *mpi_module;
  static void *PyFF_API[PyFF_API_pointers];

  /* Create the module and add the functions */
  m = Py_InitModule("_MMTK_forcefield", forcefield_methods);
  ...
  module = PyImport_ImportModule("bilab.structure.forcefields.Units");
  if (module != NULL) {
    PyObject *module_dict = PyModule_GetDict(module);
    PyObject *factor = PyDict_GetItemString(module_dict,"electrostatic_energy");
