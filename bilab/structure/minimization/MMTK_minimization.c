...
conjugateGradient(PyObject *dummy, PyObject *args)
{
  ...
  gradients2 = (PyArrayObject *)PyArray_FromDims(configuration->nd,
             configuration->dimensions, PyArray_DOUBLE);
#endif
  if (gradients2 == NULL) {
  ...
  norm_sq = 0.;
  for (i = 0; i < steps; i++) {
    // Abbruch wenn zu viele ZeroShifts
    if(ZeroShiftCount > 1000)
    {   break;
    //printf("Cycle: %i Zeroshifts: %i\n",i,ZeroShiftCount);
    }
  ...
  for(j=0; j<atoms;j++)
  {  
    if(h[j][0]>1000000 || h[j][1]>1000000 || h[j][2]>1000000) {
      reset_count=5*atoms;
    }
  }      
...
#define eval(p) \
    { \
      ...
      if(ShiftNorm < 0.000001)ZeroShiftCount++; \
      PyUniverseSpec_StateLock(universe_spec, -2); \
      PyUniverseSpec_StateLock(universe_spec, 1); \
      (*evaluator->eval_func)(evaluator, &p_energy, configuration, 0); \
      PyUniverseSpec_StateLock(universe_spec, 2); \
...
#endif
{
  ...
  m = Py_InitModule("_MMTK_minimization", minimization_methods);
  ...
  //import__MMTK_trajectory();

  PyObject *module = PyImport_ImportModule("bilab.structure.minimization._MMTK_universe"); 
  if (module != NULL) { 
  ...
  module = PyImport_ImportModule("bilab.structure.minimization._MMTK_forcefield"); 
  if (module != NULL) { 
    PyObject *module_dict = PyModule_GetDict(module); 
    PyObject *c_api_object = PyDict_GetItemString(module_dict, "_C_API"); 
    if (PyCObject_Check(c_api_object)) { 
  ...
  module = PyImport_ImportModule("bilab.structure.minimization._MMTK_trajectory"); 
  if (module != NULL) { 
    PyObject *module_dict = PyModule_GetDict(module); 
    PyObject *c_api_object = PyDict_GetItemString(module_dict, "_C_API"); 
    if (PyCObject_Check(c_api_object)) { 
