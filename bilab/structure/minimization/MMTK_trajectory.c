      ...
      int block_size)
{
  ...
  goto error;
      if (block_size > 1 && PyNetCDFFile_CreateDimension(self->file, 
                  minor_step_number,  block_size) == -1)
  goto error;
  ...
  goto error;
      if (universe_spec->geometry_data_length > 0
            && PyNetCDFFile_CreateDimension(self->file, box_size_length,
              universe_spec->geometry_data_length) == -1)
  goto error;
  ...
  goto error;
      description_var = PyNetCDFFile_CreateVariable(self->file,
            "description", 'c', &description_length, 1);
  ...
  goto error;
      ...
      if (self->var_step == NULL)
  goto error;
      if (self->cycle > 0  && PyTrajectory_SetAttribute(self, "last_step", PyInt_FromLong(-1)) == -1)
  goto error;
      self->steps = 0;
    }
  }
  ...
  Py_DECREF(step_array);
    ...
    if (mode[0] == 'a') {
      ...
      Py_ssize_t pos = 0;
      while (PyDict_Next(self->file->variables, &pos, &name, (PyObject **)&variable))
      if (variable->type == PyArray_FLOAT || variable->type == PyArray_DOUBLE) {
  ...
  goto error;
      if (!verify_description(PyString_AsString(description),
            PyString_AsString((PyObject *) traj_description))) {
...
enum PySpec_TYPE {PySpec_None, PySpec_Trajectory, PySpec_Print, PySpec_Function};
    ...
    PyTrajectoryVariable *data, int nvar)
{
  ...
  if (type == PySpec_Function) {
    ...
    output->function = (trajectory_fn *)
                     PyCObject_AsVoidPtr(PyTuple_GetItem(spec, (Py_ssize_t)4));
...
PyTrajectory_OutputSpecification(PyObject *universe, PyListObject *spec_list,
          char *description, PyTrajectoryVariable *data)
{
            ...
            PyTrajectoryVariable *data, PyThreadState **thread)
{
  ...
  while (spec->type != PySpec_None) {
    if ((step >= spec->first && step < spec->last &&
      ...
      if (spec->type == PySpec_Trajectory) {
        PyTrajectoryObject *trajectory = (PyTrajectoryObject *)spec->destination;
        PyObject **tvar;
        clock_t cpu_time;
        if (trajectory->cycle > 0 && step < 0) step = -step;
        if (step >= 0) {
          if (thread != NULL) PyEval_RestoreThread(*thread);
          if (PyTrajectory_Step(trajectory, step) == -1) return -1;
          for (var = data, tvar = spec->variables; var->name != NULL; var++, tvar++)
            if (spec->what & var->class) {
              PyArrayObject *array;
              switch (var->type) {
                case PyTrajectory_Scalar:
            ...
            if (trajectory->cycle > 0) {
              if (PyTrajectory_SetAttribute(trajectory, "last_step",
                PyInt_FromLong(trajectory->steps-1)) == -1)
                return -1;
            ...
            if (PyErr_CheckSignals()) interrupt = -1;
            if (thread != NULL) *thread = PyEval_SaveThread();
          }
      }
      if (spec->type == PySpec_Function) {
...
boxTransformation(PyObject *dummy, PyObject *args)
{
  ...
  universe_spec->trajectory_function(in, out, pt_in->dimensions[0], box, to_box);
  Py_INCREF(Py_None);
  return Py_None;
}
...
#endif
{
  ...
  module = Py_InitModule("_MMTK_trajectory", module_methods);
  ...
  module = PyImport_ImportModule("bilab.structure.minimization._MMTK_universe");
  if (module != NULL) {
  ...
  netcdf = PyImport_ImportModule("_NetCDF");
  if (netcdf != NULL) {
