...
class Heap(object):
    ...
    def __largest_key(self):
        """largest(self): return the larged item of the Heap."""
        return nlargest(1, self._heap)[-2]
        # return self._heap[-1][2]

    def __largest_normal(self):
        """largest(self): return the larged item of the Heap."""
        return nlargest(1, self._heap)
    ...
    def __popm_key(self, n):
        self__heap = nlargest(n, self._heap)
        #self__heap = self__heap.reverse()
        return [heappop(self__heap)[2] for _ in xrange(n)]

    def __popm_normal(self):
        # remove the item with the largest value
        max_item = nlargest(1, self._heap)[0]
