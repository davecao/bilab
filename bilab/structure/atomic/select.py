...
It offers a fast and powerful atom selection class, :class:`.Select`.
...
.. ipython:: python
   ...
   from bilab.structure import parsePDB
   p = parsePDB('3mht.pdb')
...
.. ipython:: python
   ...
   p.select('sqrt(sq(x) + sq(y) + sq(z)) < 100')  # within 100 angstroms of origin


Distance based selections
-------------------------------------------------------------------------------

Atoms within a user specified distance (angstroms) from a set of user specified atoms
can be selected using ``'within . of .'`` keyword, e.g. ``'within 5 of water'``
selects atoms that are within 5 angstroms of water molecules. This setting will
...
It allows you to define a macro for any valid selection string.  Below
...
.. ipython:: python
    ...
    from bilab.structure.atomic import defSelectionMacro, delSelectionMacro
...
.. ipython:: python
    ...
    from bilab.structure import calcCenter
    protein = p.protein
...
from bilab import LOGGER, SETTINGS, PY2K
...
from bilab.utilities import rangeString
...
def defSelectionMacro(name, selstr):
    ...
    *selstr* must be string.  An existing keyword cannot be used as a macro
    ...
    .. ipython:: python
       ...
       from bilab.structure.atomic import defSelectionMacro
...
def delSelectionMacro(name):
    ...
    .. ipython:: python
       ...
       from bilab.structure.atomic import delSelectionMacro
...
class Select(object):
    ...
    and examples.  This class makes use of :mod:`~.atomic.pyparsing` module.
