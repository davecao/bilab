# -*- coding: utf-8 -*-
"""This module defines a lookup table to handle individual atom names in PDB."""
#
...
AtomLookUptableColumns = ["AA", "BMRB", "SC", "PDB", "UCSF", "MSI", "XPLOR", "SYBYL*", "MIDAS*", "DIANA"]
...
def getAtomNomenclature(aa_one_letter, db="PDB"):
    """ Return atoms of an amino acid

    Args:
        aa_one_letter (str): amino acid in a single letter

    Kwargs:
        db (str): atomic nomenclatures in the following software.
                default is PDB.
            "BMRB"   -  System in use at BioMagResBank (IUPAC/IUB Biochemistry 9, 3471-3479 [1970]).
            "SC"     -  Stereochemical designations
            "PDB"    -  PDB nomenclature (Taken from PDB entry 6I1B REVDAT 15-OCT-92.)
            "UCSF"   -  Mardigras-type software (peptide protonated with newhyd utility).
            "MSI"    -  Artificial peptide created with InsightII.
            "XPLOR"  -  Peptide protonated with XPLOR 3.1.
            "SYBYL*" -  The atom nomenclature was taken from the xxx.res files supplied with
                        the software package Sybyl version 6.2 from Tripos, Inc.
            "MIDAS*" -  MidasPlus from the Computer Graphics Laboratory at UCSF.
            "DIANA"  -  NMR structure calculation. Not supported now, but incorporated into DYANA or CYANA.

    Returns:
        a list containing atomic name for the specified db.
    """

    db_inx = AtomLookUptableColumns.index(db)
    if not (isinstance(aa_one_letter, string_types) and len(aa_one_letter) == 1):
        print("The first parameter should be amino acid in a single letter")
        return None

    if db is None:
        print("Error: db={} is not found".format(db))
        return None
    aa_atoms = [ tbl[db_inx] for tbl in AtomLookUpTable if tbl[0] == aa_one_letter ]
    return aa_atoms