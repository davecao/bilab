            ...
            real_t *tpwgts, real_t *ubvec)
{
  ...
  switch (optype) {
    case METIS_OP_PMETIS:
      ctrl->objtype = (int)GETOPTION(options, METIS_OPTION_OBJTYPE, METIS_OBJTYPE_CUT);
      ...
      if (ncon == 1) {
        ctrl->iptype    = (int)GETOPTION(options, METIS_OPTION_IPTYPE,  METIS_IPTYPE_GROW);
        ctrl->ufactor   = GETOPTION(options, METIS_OPTION_UFACTOR, PMETIS_DEFAULT_UFACTOR);
        ctrl->CoarsenTo = 20;
      }
      else {
        ctrl->iptype    = (int)GETOPTION(options, METIS_OPTION_IPTYPE,  METIS_IPTYPE_RANDOM);
    ...
    case METIS_OP_OMETIS:
      ctrl->objtype  = (int)GETOPTION(options, METIS_OPTION_OBJTYPE,  METIS_OBJTYPE_NODE);
      ctrl->rtype    = (int)GETOPTION(options, METIS_OPTION_RTYPE,    METIS_RTYPE_SEP1SIDED);
      ctrl->iptype   = (int)GETOPTION(options, METIS_OPTION_IPTYPE,   METIS_IPTYPE_EDGE);
  ...
  /* common options */
  ctrl->ctype   = (int)GETOPTION(options, METIS_OPTION_CTYPE, METIS_CTYPE_SHEM);
  ctrl->no2hop  = GETOPTION(options, METIS_OPTION_NO2HOP, 0);
  ctrl->seed    = GETOPTION(options, METIS_OPTION_SEED, -1);
  ctrl->dbglvl  = (int)GETOPTION(options, METIS_OPTION_DBGLVL, 0);
