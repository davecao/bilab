    ...
    if (preflen+1 >= params->minlen)
      (*params->callback)(
           params->stateptr, preflen+1, prefix,
              (int)(mat->colptr[(int)i+1] - mat->colptr[(int)i]),
                          mat->colind+mat->colptr[(int)i]);

    if (preflen+1 < params->maxlen) {
      cmat = itemsets_project_matrix(params, mat, (int)i);
...
gk_csr_t *itemsets_project_matrix(isparams_t *params, gk_csr_t *mat, int cid)
{
  ...
  pmat = gk_csr_Create();
  pmat->nrows  = pnrows = (cid == -1 ? nrows : (int)(colptr[cid+1]-colptr[cid]));
