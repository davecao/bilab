...
{
  ssize_t i, k, l;
  size_t nfields, nvtxs, nedges, fmt, ncon = 0, lnlen;
  int32_t ival;
  float fval;
  ...
  for (graph->xadj[0]=0, k=0, i=0; i<nvtxs; i++) {
    ...
    if (readsizes) {
      if (isfvsizes) {
        ...
        graph->ivsizes[i] = (int)strtol(head, &tail, 0);
        if (tail == head)
          gk_errexit(SIGERR, "The line for vertex %zd does not have size information\n", i+1);
    ...
    if (readwgts) {
      for (l=0; l<ncon; l++) {
        if (isfvwgts) {
          ...
          graph->ivwgts[i*ncon+l] = (int)strtol(head, &tail, 0);
          if (tail == head)
            gk_errexit(SIGERR, "The line for vertex %zd does not have enough weights "
                    "for the %d constraints.\n", i+1, ncon);
...
int gk_graph_FindComponents(gk_graph_t *graph, int32_t *cptr, int32_t *cind)
{
  ...
  while (ntodo > 0) {
    if (first == last) { /* Find another starting vertex */
      cptr[++ncmps] = (int)first;  /* Mark the end of the current CC */
          ...
          int32_t **r_perm, int32_t **r_iperm)
{
  ...
  */
  ...
  for (i=0; i<nvtxs; i++) {
    if ((v = (int)gk_i32pqGetTop(queue)) == -1)
...
void gk_graph_SingleSourceShortestPaths(gk_graph_t *graph, int v, void **r_sps)
{
  ...
  if (graph->iadjwgt != NULL) {
    ...
    /* start processing the nodes */
    while ((v = (int)gk_i32pqGetTop(queue)) != -1) {
      inqueue[v] = 2;

      /* relax the adjacent edges */
      for (i = (int)xadj[v]; i<xadj[v+1]; i++) {
        u = adjncy[i];
        if (inqueue[u] == 2)
          continue;
    ...
    *r_sps = (void *)sps;
    ...
    /* start processing the nodes */
    while ((v = (int)gk_fpqGetTop(queue)) != -1) {
      inqueue[v] = 2;
