...
gk_csr_t *gk_csr_LowFilter(gk_csr_t *mat, int what, int norm, float fraction)
{
  ...
  switch (what) {
    case GK_CSR_COL:
      ...
      for (i=0; i<ncols; i++) 
        maxlen = (int)gk_max(maxlen, colptr[i+1]-colptr[i]);
      ...
      #pragma omp parallel private(i, j, ncand, rsum, tsum, cand)
      {
        ...
        for (i=0; i<ncols; i++) {
          ...
          for (rsum=0.0, j=0; j<ncand && rsum<=fraction*tsum; j++) {
            ...
            nrowind[nrowptr[cand[j].val]] = (int)i;
    ...
    case GK_CSR_ROW:
      ...
      for (i=0; i<nrows; i++) 
        maxlen = (int)gk_max(maxlen, rowptr[i+1]-rowptr[i]);
      ...
      #pragma omp parallel private(i, j, ncand, rsum, tsum, cand)
      {
        ...
        for (i=0; i<nrows; i++) {
          ...
          for (rsum=0.0, j=0; j<ncand && rsum<=fraction*tsum; j++) {
            rsum += (norm == 1 ? cand[j].key : cand[j].key*cand[j].key);
            nrowind[rowptr[i]+j] = (int)cand[j].val;
            nrowval[rowptr[i]+j] = (int)cand[j].key;
          }
  ...
  switch (what) {
    ...
    case GK_CSR_ROW:
      ...
      for (nnz=0, i=0; i<nrows; i++) {
        ...
        k = gk_min(topk, ncand);
        for (j=0; j<k; j++, nnz++) {
          nrowind[nnz] = (int)cand[j].val;
          nrowval[nnz] = (int)cand[j].key;
        }
        for (; j<ncand; j++, nnz++) {
          if (cand[j].key < keepval) 
            break;
...
void gk_csr_SortIndices(gk_csr_t *mat, int what)
{
  ...
  #pragma omp parallel if (n > 100)
  {
    ...
    #pragma omp single
    for (i=0; i<n; i++) 
      nn = (int)gk_max(nn, ptr[i+1]-ptr[i]);
  ...
  switch (type) {
    ...
    case GK_CSR_IDF2: /* TF' = TF*IDF */
      ...
      #pragma omp parallel if (ncols > OMPMINOPS) 
      {
        ...
        bgfreq = (int)gk_max(10, (ssize_t)(.5*rowptr[nrows]/nnzcols));
...
float gk_csr_ComputeSimilarity(gk_csr_t *mat, int i1, int i2, int what, int simtype)
{
  ...
  switch (what) {
    case GK_CSR_ROW:
      if (!mat->rowptr)
        gk_errexit(SIGERR, "Row-based view of the matrix does not exists.\n");
      nind1 = (int)(mat->rowptr[i1+1] - mat->rowptr[i1]);
      nind2 = (int)(mat->rowptr[i2+1] - mat->rowptr[i2]);
    ...
    case GK_CSR_COL:
      if (!mat->colptr)
        gk_errexit(SIGERR, "Column-based view of the matrix does not exists.\n");
      nind1 = (int)(mat->colptr[i1+1]-mat->colptr[i1]);
      nind2 = (int)(mat->colptr[i2+1]-mat->colptr[i2]);
