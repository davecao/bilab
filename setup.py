...
class Find_BOOST_PACKAGE(object):
    ...
    def setupLibrarySettings(self):
        ...
        if self.incDir and (not os.path.isdir(pj(self.incDir,'boost'))):
            print("Boost inc dir is not a valid directory: {}".format(pj(self.incDir,'boost')))
            return
        # Check the version header is there         
        version_header = pj(self.incDir, 'boost', 'version.hpp')
        if not os.path.isfile(version_header):
            print("Boost version.hpp header does not exist:{}".format(version_header))
            return
        ...
        if not ver_match:
            print("   could not find BOOST_VERSION in file: {}".format(version_header))
            return
        ...
        if found_ver < req_ver:
            print("   found version is to old: required:{} found:{}"%(self.minimal_version,found_ver_str))
            return
